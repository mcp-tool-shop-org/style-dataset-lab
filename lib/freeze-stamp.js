 ...
 *
 * A missing `freeze` block, or a `freeze` block with no `status` key at all,
 * collapses to `auto` — that permissive default is deliberate (D8): an entry
 * that has never opted into freeze protection should never block a write.
 *
 * But a `freeze` block that IS present with a `status` value that is a
 * non-empty string outside `FREEZE_STATUSES` is not "absent" — it is
 * malformed. Coercing that case to `auto` too would silently disable hard
 * freeze protection on a hand-edited typo (e.g. `status: Frozen` against the
 * lowercase-only enum) with no error, no warning, and no bypass event, since
 * the bypass path is never entered. Fail loud instead: throw so the operator
 * fixes the typo rather than unknowingly shipping an unprotected entry.
 *
 * @throws {SdlabError} CANON_FREEZE_STATUS_INVALID when `freeze` is present
 *   and `status` is a non-empty string that isn't a legal FREEZE_STATUSES value.
 */
export function readFreezeStatus(frontmatter) {
  const status = frontmatter?.freeze?.status;
  if (typeof status !== 'string') return 'auto';
  if (FREEZE_STATUSES.includes(status)) return status;
  if (status.length > 0) {
    throw inputError(
      'CANON_FREEZE_STATUS_INVALID',
      `Invalid freeze.status "${status}"${frontmatter?.id ? ` on entry "${frontmatter.id}"` : ''}: ` +
        `not one of [${FREEZE_STATUSES.join(', ')}].`,
      `Fix the freeze.status value (check for a case typo — statuses are lowercase-only). ` +
        `A malformed status is treated as an error, not as "auto", so a typo can never ` +
        `silently disable freeze protection.`,
    );
  }
  return 'auto';
}
