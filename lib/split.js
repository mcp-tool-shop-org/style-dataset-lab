 ...
 *
 * F7: exported (previously module-private) so its O(n) behavior can be
 * measured directly in tests without paying for a full
 * createSnapshot+createSplit round trip's file I/O on every record —
 * buildAncestorMap itself is a pure function of `records` (no I/O, no
 * side effects), so this is a safe, low-risk addition to the module's
 * export surface purely for testability.
 */
export function buildAncestorMap(records) {
  ...
  // F7: memoize resolved roots into a map shared across every findRoot()
  // call below, as the walk proceeds. Before this, findRoot() got a FRESH
  // per-call `visited` Set with no memoization shared across calls — for a
  ...
  // record 1, record 3 derived from record 2, ...), the outer loop below
  // calls findRoot() once per record, and EACH call re-walked an
  // ever-longer prefix of the same chain from scratch — resolving all k
  // records cost ~k²/2 lookups instead of k.
  //
  // rootCache maps id -> its fully-resolved root id. Every id visited
  // during a walk gets pinned to the same final root once found (path
  // compression): the first call to walk the whole chain still costs O(k),
  // but every subsequent call for any id already visited on that walk (or
  // whose walk crosses a previously-cached id partway through) is an O(1)
  // cache hit instead of a re-walk.
  //
  // Written as an explicit loop rather than recursion: profiling this fix
  // against a synthetic 20,000-record linear chain found the ORIGINAL
  // recursive walk (recursion depth == chain length, once per call) blows
  // V8's call stack ("RangeError: Maximum call stack size exceeded") well
  // before memoization would even matter — a chain long enough to make the
  // O(n^2) cost bite is also long enough to crash outright. Fixing the
  // algorithmic complexity without also fixing the recursion-depth ceiling
  // would leave a strictly worse failure mode (a crash) sitting right next
  // to the fix, so both are closed together here.
  //
  // This does not change results for the acyclic chains this field
  // actually models — for a pathological cycle (which
  // lineage.derived_from_record_id should never legitimately contain) it
  // still terminates safely and deterministically; it just picks whichever
  // member of the cycle the outer loop reaches first as the shared "root"
  // for the whole cycle, rather than each cyclic member resolving to
  // itself independently the way a fully-independent (non-memoized) walk
  // would. Cycle protection itself (a node revisited within the SAME walk)
  // is otherwise semantically unchanged from the original.
  ...
  function findRoot(startId) {
    ...
    // Walk forward from startId, collecting the path as we go, until we
    // hit: (a) a node with an already-cached root (reuse it — this is the
    // O(1) fast path for everything after the first walk through a given
    // chain), (b) a node with no parent (it IS the root), or (c) a node
    // already seen ON THIS WALK (a cycle — stop there, matching the
    // original recursive version's "cycle stops at the repeated node"
    // behavior exactly).
    const path = [];
    ...
    while (true) {
      ...
      if (onPath.has(current)) {
        root = current; // cycle — stop at the repeated node
        break;
      }
      onPath.add(current);
      path.push(current);
    ...
    // Path compression: pin every node visited on THIS walk directly to
    // the resolved root, so any future call for any of them — from this
    // top-level findRoot() call or a later one — is an O(1) lookup.
...
export async function createSplit(projectRoot, snapshotId, profile, options = {}) {
  ...
  // Declared here (rather than where the pre-existing lane/family warnings
  // are pushed further below) so the F3 missing-records warning shares the
  // same array — and therefore the same manifest.json / audit.json
  // `warnings` field — as every other warning this function produces.
  const warnings = [];

  // Load full records for included entries.
  //
  // F3: a record the (frozen, Law-1) snapshot included whose file was
  // later deleted from records/ must never silently drop out of the split
  // with no counter, no warning, no id recorded. loadRecord() returns null
  // on ENOENT (lib/records.js); previously `if (record) records.push(record)`
  // was the ENTIRE handling for that case — a missing record just shrank
  // `records` by one, invisibly. WARN + ledger here rather than hard-fail:
  // a split is an intermediate artifact, not itself the Law-4
  // ("exports are reproducible") contract-bearing one — export.js and
  // training-packages.js are, and hard-fail on the identical situation
  // (see their EXPORT_MISSING_RECORDS / TRAINING_PACKAGE_MISSING_RECORDS
  // guards) rather than warning.
  const records = [];
  ...
  for (const entry of includedEntries) {
    ...
    if (record) {
      ...
      missingRecords.push({ record_id: entry.record_id, stage: 'split' });
    }
  }
  if (missingRecords.length > 0) {
    const sample = missingRecords.slice(0, 10).map(m => m.record_id);
    const msg =
      `split: ${missingRecords.length} of ${includedEntries.length} record(s) referenced by snapshot ` +
      `"${snapshotId}" could not be loaded from ${recordsDir} — this split is built from the ` +
      `${records.length} record(s) actually found` +
  ...
  // F3: `warnings` is declared once, up near the top of createSplit (ahead
  // of the missing-records check), and reused here — NOT redeclared — so
  // every warning this function produces (missing records, lane-family
  // count, guessed-family share, stem-collision) lands in one array and
  // therefore one manifest.json / audit.json `warnings` field.
  ...
  const manifest = {
    ...
    counts: {
      ...
      // F3: how many snapshot-included records could not be loaded from
      // records/ at split time. 0 in the common case; see missing_records
      // below for the actual ids.
      missing_records: missingRecords.length,
    },
    // F3: ledger of every snapshot-included record_id that failed to load
    // (mirrors the images_failed pattern export.js established one layer
    // down at export time) — record_id + which stage recorded the miss, so
    // an operator can trace it without re-diffing snapshot vs. records/.
