...
export function applySlotDelta(baseBrief, slot) {
  ...
  // Negative additions — append without duplicating (token-exact match)
  if (slot.negative_additions?.length > 0) {
    const existingLower = modified.negative_prompt
      .toLowerCase()
      .split(',')
      .map(s => s.trim())
      .filter(s => s.length > 0);
    const additions = slot.negative_additions.filter(
      n => !existingLower.includes(n.toLowerCase().trim())
    );
