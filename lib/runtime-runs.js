...
import { verbose, warn } from './log.js';
...
// SDL-H7: the only shape a run id can ever legitimately have, mirroring the
// prefix/date/seq scheme generateRunId() builds above. Validated BEFORE the
// id is joined into a path, so `--resume ../../../../some/dir/leftover`
// (or any other traversal payload) never reaches existsSync/readFile. A
// mismatch is reported as the SAME RUN_NOT_FOUND (not a distinct code)
// because no real run can ever have an id outside this shape — the two
// cases are indistinguishable to the caller, and this avoids leaking
// validation internals to whoever is probing.
const RUN_ID_RE = /^run_\d{4}-\d{2}-\d{2}_\d{3}$/;
 ...
 *
 * B01: this used to `continue` past a directory with no readable
 * manifest.json with zero signal — the exact shape a crashed/killed run
 * leaves behind (see checkpointRunManifest() above). A run that ate 90
 * minutes of GPU time before dying became invisible: no id, no `run show`,
 * no resume, and no indication anything was ever skipped. Skipped/unreadable
 * directories are now reported: a one-line warn() with a count always
 * (so a human running `sdlab run list` cannot miss it), and the per-directory
...
export function listRuns(projectRoot) {
  ...
  for (const entry of entries) {
    ...
    if (!existsSync(manifestPath)) {
      skipped.push({ name: entry.name, reason: 'no manifest.json' });
      continue;
    }
    ...
    try {
      ...
      skipped.push({ name: entry.name, reason: `unreadable manifest.json (${err.message})` });
    }
  }

  if (skipped.length > 0) {
    warn(
      `listRuns: skipped ${skipped.length} run director${skipped.length === 1 ? 'y' : 'ies'} ` +
      `with no readable manifest.json (run with --verbose for details)`
    );
    for (const s of skipped) {
      verbose(`  skipped run dir "${s.name}": ${s.reason}`);
    }
  }
