...
export async function runBuild(opts) {
  ...
  // Entity ids must be unique across the ENTIRE canon tree, not just within
  // one schema's subdirectory: they key prompts/<id>.j2, context/<id>.md, and
  // manifest.per_entity_hashes, all of which are flat maps keyed by id alone.
  // Two entries sharing an id (e.g. a forgotten `id:` edit after cloning an
  // existing file) would silently overwrite each other's prompt/context
  // output and collide in the manifest (last write wins), while
  // dataset/all.jsonl would keep rows for both under the one id — so a row
  // permanently points at the wrong entry's content. Catch this at discovery
  // time, before any output is written.
  const seenIds = new Map(); // id -> first-seen filePath
  ...
  for (const [schemaName, subdir] of Object.entries(config.entity_dirs)) {
    ...
    for (const entry of loaded) {
      ...
      if (seenIds.has(id)) {
        throw inputError(
          'CANON_DUPLICATE_ENTITY_ID',
          `Duplicate canon entity id "${id}": both ${seenIds.get(id)} and ${entry.filePath} ` +
            `resolve to this id.`,
          `Give one of these entries a unique "id:" field in its frontmatter. IDs must be ` +
            `unique across the whole canon tree — they key prompts/, context/, and ` +
            `manifest.per_entity_hashes, all flat maps with no per-schema namespacing.`,
        );
      }
  ...
  for (const { entry, schemaName, id } of entries) {
    ...
    // Build the row. `id` is the RESOLVED id (frontmatter.id, falling back to
    // the filename stem — see entryId() in load-entry.js), which is what the
    // prompts/<id>.j2 and context/<id>.md files below are actually named
    // with. canonEntryToRow() (lib/rows.js) reads `entry.id` for entity_id —
    // passing the raw entry.frontmatter would leave entity_id undefined for
    // any entry that relies on the filename-stem fallback (no explicit
    // `id:` field), and serializeRow() drops undefined fields entirely, so
    // the row would orphan itself from its own correctly-named prompt/context
    // files. Overlay the resolved id so the row always carries it.
  ...
  //
  // M2: everything from here through the rename is wrapped in try/catch.
  // Before this fix, a throw ANYWHERE in this block (most notably
  // readFreezeStatus() below, which throws CANON_FREEZE_STATUS_INVALID on a
  // malformed freeze.status) left `stagingDir` — by then already populated
  // with dataset/prompts/context — orphaned on disk forever: the only
  // cleanup is the `rm(stagingDir, ...)` at the top of THIS function, and
  // that runs against a NEW `${generatedFromDir}.tmp-${process.pid}` path on
  // the next invocation (a different pid), so it can never match the
  // orphan. The catch below removes the in-progress staging directory
  // before rethrowing, and does not touch anything once rename() has
  // succeeded (stagingDir no longer exists at that point, so the block
  // exits normally and the catch never runs).
  await rm(stagingDir, { recursive: true, force: true });
  // Declared here (not inside the try block) because they're read again
  // after it, in writeLatestSchemaVersions() and the return value below.
  let frozenEntriesHashes = {};
  let drift = [];
  let manifest;
  try {
    ...
    try {
      await rm(stagingDir, { recursive: true, force: true });
    } catch { /* best-effort cleanup — surface the original error below regardless */ }
    throw err;
  }
