...
import { buildBaseRecord, buildIdentityRecord } from './records.js';
...
// 1.1.0: adds `notices` (provenance gaps reported, not gated) and the
// `tone_operands` file role. Additive — a 1.0.0 receipt reads fine.
export const ASSET_RECEIPT_SCHEMA_VERSION = '1.1.0';
...
export async function ingestAssetSource(projectRoot, sourceDir, options = {}) {
  ...
  if (existsSync(receiptPath)) {
    ...
    if (receipt.manifest_sha256 === manifestSha) {
      // Notices replay from the stored receipt rather than being recomputed —
      // a re-run must report the same provenance gaps the original ingest did,
      // and a receipt written before 1.2.0 honestly has none to report.
      return { assetId, alreadyIngested: true, receiptPath: toPortablePath(join('assets', assetId, RECEIPT_FILENAME)), created: [], skipped: (receipt.records_created || []).slice(), rejected: receipt.rejected_renders || [], notices: receipt.notices || [], dryRun };
    }
    throw inputError(
  ...
  for (const [renderId, entry] of resolved.renders) {
    ...
    // Class shares are a PIXEL measurement. Since schema 1.1.0 an instance
    // may be an npy array or a json sidecar; those carry their class facts
    // asset-side (facet's admission sidecars already count them) and sdlab
    // does not recompute what it would have to interpret to recompute.
    const shares = {};
    for (const [cid, inst] of entry.instances) {
      const decl = channelDecls.get(cid);
      if (!decl?.categorical || !PIXEL_ENCODINGS.has(decl.encoding)) continue;
  ...
  const subjectName = manifest.identity?.subject_name ?? null;

  // ── Asset-level style + tone provenance (schema 1.2.0) ─────────────
  // Resolved ONCE here and copied onto every record, so a consumer never has
  // to re-open the manifest (or re-derive a default) to know what register a
  // record was painted in or whether a tone map sits under its colours.
  const style = manifest.asset.style ?? null;
  const styleRecord = style
    ? {
        register_terms: [...style.register.terms],
        register_ruling: style.register.ruling ?? null,
        register_record: style.register.record ?? null,
        lora: style.lora.declared === 'none'
          ? { declared: 'none', card: null, weight: 0 }
          : { declared: 'card', card: style.lora.card, weight: style.lora.weight ?? null },
      }
    : null;

  const toneDecl = manifest.asset.tone_transform ?? null;

  // schema 1.3.0 — how these renders came to exist. `generated === false`
  // means an absent generation block is CORRECT by declaration rather than a
  // gap, so the notice below must not report one.
  const derivationDecl = manifest.asset.render_derivation ?? null;
  const derivationRecord = derivationDecl
    ? { kind: derivationDecl.kind, generated: derivationDecl.generated, record: derivationDecl.record ?? null }
    : null;
  const plans = [];
  const skipped = [];
  ...
  // ── Provenance notices (schema 1.2.0) ──────────────────────────────
  // NOT gates. Every one of these describes a manifest that is contract-legal
  // and ingests fine — a 1.0.0 export declares none of it, and E11 Ruling 2
  // named those exports as the training input, so refusing them is not on the
  // table. What IS on the table is refusing to be quiet: an absent register or
  // an unknown seed is a fact about the training data, and a fact nobody is
  // told is one nobody can act on. These ride in the receipt and print at the
  // CLI.
  //
  // Two kinds, and the difference matters to anything that reads them:
  //   'gap'  — the manifest did NOT declare something a curator needs. Closing
  //            it is export-side work.
  //   'info' — the manifest DID declare something that changes how these
  //            records should be read. Nothing to fix; something to know.
  const notices = [];
  if (!styleRecord) {
    notices.push({
      code: 'ASSET_STYLE_UNDECLARED',
      kind: 'gap',
      message: 'No asset.style — the register this subject was painted in is UNKNOWN to the lane. ' +
        'Per facet E12 Ruling 10b the style register is subject data; a training pool mixing declared and undeclared registers cannot be filtered by it. ' +
        'Records carry style: null, which is distinguishable from a ruled no-LoRA register.',
    });
  }
  if (toneDecl) {
    const optedOut = (manifest.renders || []).filter((r) => r.tone_transform === false).length;
    notices.push({
      code: 'ASSET_TONE_TRANSFORM_DECLARED',
      kind: 'info',
      message: `Renders derive from sources carrying a declared "${toneDecl.kind}" tone transform toward ${toneDecl.reference}` +
        (optedOut > 0 ? ` (${optedOut} render(s) opted out)` : '') +
        '. sdlab records the declaration and hashes the operands; it does NOT verify the transform was applied or is reversible. ' +
        'Colour measurements on these records — palette gate, class shares, ΔE — sit downstream of it.',
    });
  }
  // The generation notice is suppressed — not weakened — when the manifest
  // DECLARES its renders are deterministic derivations. An emit render of an
  // already-accepted asset has no seed; reporting its absence as a gap would
  // tell an operator to go fix an export that is correct by ruling (facet E11
  // Ruling 2), and a notice channel that cries wolf on correct data stops
  // being read. An UNDECLARED manifest still gets the gap: saying nothing is
  // not the same as saying "these are derivations."
  if (derivationDecl?.generated === false) {
    notices.push({
      code: 'ASSET_RENDERS_ARE_DERIVATIONS',
      kind: 'info',
      message: `Renders are declared "${derivationDecl.kind}" derivations of the accepted asset, not generated images — so they carry no seed or generation frame, correctly. ` +
        'Records carry generation: null by declaration rather than by omission. ' +
        'The generating provenance of any twins these were projected from belongs to those twins, not to these records.',
    });
  } else {
    const noGeneration = admitted.filter((a) => !a.entry.render.generation).length;
    if (noGeneration > 0) {
      notices.push({
        code: 'ASSET_GENERATION_PROVENANCE_ABSENT',
        kind: 'gap',
        message: `${noGeneration} of ${admitted.length} admitted render(s) declare no generation block — frame and seed are unknown for them. ` +
          'Both are measured curation axes (E12 Ruling 24b: crop-framed generation drifts register, 3 instances; Ruling 20a: term binding is seed-dependent). ' +
          'Records carry generation: null. ' +
          'If these renders are deterministic derivations rather than generated images, declare asset.render_derivation with generated: false and this stops being a gap.',
      });
    }
  }
  if (!subjectName) {
    notices.push({
      code: 'ASSET_SUBJECT_NAME_ABSENT',
      kind: 'gap',
      message: 'No identity.subject_name — lib/split.js will fall back to id-stem guessing for these records. ' +
        'Two ingests of ONE subject then read as two families, and the same subject can land in both train and test. ' +
        'Declare it before any split is cut.',
    });
  }
  ...
  // Tone-transform operands (schema 1.2.0) are MATERIALIZED, unlike the mesh,
  // atlas and texture channels beside them. They are small, and they are the
  // only record of what the upstream transform actually did to these colours
  // — leaving them as a pointer would make the lane's audit trail depend on
  // the export tree surviving. Copying them costs kilobytes and removes that
  // dependency.
  let toneOperandsRef = null;
  if (resolved.toneOperands) {
    const opName = basename(resolved.toneOperands.relPath);
    const opDest = join(assetDir, opName);
    const opRel = toPortablePath(join('assets', assetId, opName));
    const { hash, bytes } = await hashAndMaybeCopy(resolved.toneOperands.abs, opDest, true);
    note('tone_operands', resolved.toneOperands.relPath, opRel, true, hash, bytes);
    written.push(opRel);
    toneOperandsRef = { path: opRel, sha256: hash };
  }
  ...
  for (const p of plans) {
    ...
    const provenance = {
      ...
      // ── schema 1.2.0 ────────────────────────────────────────────────
      // `style` is null when the manifest declared none. That null is
      // MEANINGFUL and is why the field is always written: a record whose
      // register is unknown is distinguishable from one ruled no-LoRA, and a
      // curator can filter on it. Silence would collapse the two — the exact
      // accident E12 Ruling 10b was bought to stop.
      style: styleRecord,
      // The RESOLVED per-render answer, never a default the consumer re-derives:
      // a render carries the asset's tone transform unless it opts out.
      tone_transform: toneDecl
        ? {
            applied: render.tone_transform !== false,
            kind: toneDecl.kind,
            space: toneDecl.space ?? null,
            scope: toneDecl.scope ?? null,
            reference: toneDecl.reference,
            reference_is_self: toneDecl.reference === p.renderId,
            reversible: toneDecl.reversible ?? null,
            record: toneDecl.record ?? null,
            operands: toneOperandsRef,
          }
        : null,
      // How this render came to exist (schema 1.3.0). Load-bearing beside
      // `generation` below: with `generated: false`, a null generation is the
      // asset's DECLARATION that no seed exists, not a missing field. A
      // curator filtering for unknown-seed records must not sweep up emit
      // derivations that never had one.
      render_derivation: derivationRecord,
      // Per-image curation axes. Both measured phenomena (register drift at
      // crop frames; seed-dependent term binding) are per-image, so a record
      // that cannot answer "what frame, what seed" cannot be curated on them.
      generation: render.generation
        ? {
            frame: render.generation.frame,
            frame_detail: render.generation.frame_detail ?? null,
            seed: render.generation.seed ?? null,
            stem: render.generation.stem ?? null,
            model: render.generation.model ?? null,
            reroll_of: render.generation.reroll_of ?? null,
          }
        : null,
    };
    ...
    // identity.subject_name (schema 1.1.0) is the split engine's only
    // AUTHORED subject-family key. Without it lib/split.js falls back to
    // id-stem guessing, which reads two ingests of one subject as two
    // families and will happily put the same subject in train and test.
    // Lineage is null: an asset render is not derived from another record.
    const record = subjectName
      ? buildIdentityRecord(base, { subject_name: subjectName }, null)
      : base;
