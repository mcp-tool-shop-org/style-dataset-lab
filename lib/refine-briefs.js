...
export function renderRefinedNegative(parentNegative, delta) {
  const existing = parentNegative.split(',').map(s => s.trim().toLowerCase());
  const additions = [];

  for (const item of delta.suppress || []) {
    const lower = item.toLowerCase();
    // Don't duplicate if already in negative
    if (!existing.some(e => e.includes(lower) || lower.includes(e))) {
      additions.push(item);
    }
  }

  if (additions.length === 0) return parentNegative;
  return `${parentNegative}, ${additions.join(', ')}`;
}
