...
export async function getWorkflowProfile(projectRoot, workflowId) {
  ...
  if (!existsSync(path)) {
    throw inputError(
      ...
      `Workflow profile "${workflowId}" not found at ${path}`,
      'Check the workflow id, or run: sdlab workflow list'
    );
  }
  // B06: JSON.parse used to run unguarded here, so a malformed profile file
  ...
  try {
    ...
    throw inputError(
      'INPUT_INVALID_JSON',
      `Failed to parse ${path}: ${err.message}`,
  ...
  if (!valid) {
    throw inputError(
      ...
      `Workflow profile "${workflowId}" is invalid:\n  ${errors.join('\n  ')}`,
