 ...
 *
 * ── Schema 1.3.0 ───────────────────────────────────────────────────────
 * One addition, closing a defect 1.2.0 shipped with. Per facet E11 Ruling 2
 * (endorsed as E11 addenda 3), a dense turnaround's renders are
 * DETERMINISTIC DERIVATIONS of the accepted asset by the anchored emit path
 * — not generated images. They therefore carry no `generation` block, and
 * *correctly* so: "an optional block that does not apply is left out, not
 * filled in."
 *
 * 1.2.0 could not tell that apart from an export that simply failed to
 * declare its seeds, so it reported the gap on both — which would have told
 * an operator to go fix an export that was right by ruling. Crying wolf on
 * correct data is how a notice channel stops being read.
 *
 *   - `asset.render_derivation` — {kind, generated, record}. `generated` is
 *                              the load-bearing bit, and it is the same
 *                              silence-vs-declaration principle as
 *                              `style.lora.declared`: the manifest SAYS its
 *                              renders are derivations rather than leaving
 *                              the lane to infer it from an absence.
 *
 *                              When `generated: false`, a per-render
 *                              `generation` block is REFUSED — a
 *                              deterministic derivation has no seed of its
 *                              own, so declaring one is a category error
 *                              worth catching rather than recording. The
 *                              generating provenance of the twins such a
 *                              render was projected FROM lives on the asset
 *                              side; it is not this render's.
 *
 * Optional, so 1.0.0–1.2.0 manifests validate unchanged; hence 1.3.0, not
 * 2.0.0. Undeclared keeps 1.2.0's behaviour exactly — the gap notice still
 * fires — because an export that declares nothing genuinely has not said
 * whether its renders were generated.
 */
...
export const ASSET_SCHEMA_VERSION = '1.3.0';
...
/**
 * Validate `asset.render_derivation` (schema 1.3.0) — how these renders came
 * to exist. Optional; complete once present.
 *
 * The point of the block is `generated`. A dense turnaround's renders are
 * deterministic derivations of an already-accepted asset (facet E11 Ruling
 * 2), so they have no seed and no generation frame — and an export that
 * leaves `generation` out for that reason is CORRECT, where an export that
 * leaves it out because nobody recorded the seeds is not. Only the manifest
 * can tell those apart, so it says which.
 *
 * @param {Object} rd — the manifest's asset.render_derivation
 * @param {Function} bad — (jsonPath, message) => void
 */
function validateRenderDerivation(rd, bad) {
  if (typeof rd !== 'object' || Array.isArray(rd)) {
    bad('asset.render_derivation', 'must be an object {kind, generated, record?}');
    return;
  }
  if (!isNonEmptyString(rd.kind)) {
    bad('asset.render_derivation.kind',
      'required non-empty string naming the path these renders came by (e.g. "emit") — sdlab does not interpret it, it records and groups by it');
  }
  if (typeof rd.generated !== 'boolean') {
    bad('asset.render_derivation.generated',
      'required boolean — true when these renders are generated images (seeds and frames apply), false when they are deterministic derivations of an already-accepted asset (no seed of their own). This is a declaration, not an inference from whether a generation block happens to be present.');
  }
  if (rd.record != null && !isNonEmptyString(rd.record)) {
    bad('asset.render_derivation.record', 'must be a non-empty string when present (where this derivation path is recorded)');
  }
}
...
export function validateManifestShape(m) {
  ...
  if (!m.asset || typeof m.asset !== 'object') {
    ...
    // schema 1.3.0 — how these renders came to exist (E11 Ruling 2)
    if (m.asset.render_derivation != null) validateRenderDerivation(m.asset.render_derivation, bad);
  }
  ...
  if (!Array.isArray(m.renders) || m.renders.length === 0) {
    ...
    m.renders.forEach((r, i) => {
      ...
      // schema 1.3.0 — refused outright on a declared non-generated render:
      // a deterministic derivation of an accepted asset has no seed and no
      // generation frame of its own. The generating provenance of the twins
      // it was projected FROM is real, but it belongs to those twins, and
      // recording it here would attribute a seed to an image that no seed
      // produced.
      if (r.generation != null && m.asset?.render_derivation?.generated === false) {
        bad(`${p}.generation`,
          `declares generation provenance, but asset.render_derivation.generated is false — these renders are "${m.asset.render_derivation.kind}" derivations of an already-accepted asset and have no seed of their own. If they ARE generated images, set generated: true.`);
      } else if (r.generation != null) {
        validateGeneration(r.generation, p, bad);
      }
