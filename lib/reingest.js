...
export async function reingestGenerated(projectRoot, sourcePath, manifestId, options = {}) {
  ...
  // D-020: ensure outputs/candidates/ exists so the record's asset_path
  // contract ("outputs/candidates/<file>") actually points at the image.
  ...
  for (const file of imageFiles) {
    ...
    if (!options.dryRun) {
      // D-020: copy the image into outputs/candidates/ so the record's
      // project-relative asset_path contract is satisfied. Without this,
      // downstream readers (export, adapters) would silently skip the
      // record because the file does not exist at the declared path.
