...
import { appendEvent, readEventsFor } from './freeze-events.js';
import { inputError } from './errors.js';
import { warn } from './log.js';

/**
 * Best-effort freeze date for an entry: the `at` of its most recent freeze /
 * override event. Returns null when the log is absent/unreadable (the freeze
 * block itself carries no timestamp). Used only to enrich the contrastive
 * refuse message — never load-bearing for the gate decision.
 */
async function latestFreezeAt(projectRoot, id) {
  try {
    const events = await readEventsFor(projectRoot, id);
    for (let i = events.length - 1; i >= 0; i--) {
      if (events[i].type === 'freeze' || events[i].type === 'override') return events[i].at;
    }
  } catch { /* no log / unreadable → no date */ }
  return null;
}

/**
 * Contrastive lead (Horvitz CHI 1999): state the DEFAULT action and the system's
 * reasoning — who froze it, when, and why — before the override path. This is the
 * "you probably expected to just regenerate; I'm refusing, here's why" framing.
 */
function contrastiveLead(id, status, freeze, frozenAt) {
  const by = freeze?.frozen_by || 'unknown';
  const reason = freeze?.frozen_reason || '(no reason recorded)';
  const when = frozenAt ? ` on ${String(frozenAt).slice(0, 10)}` : '';
  return `Default: REFUSE — "${id}" is ${status} frozen by ${by}${when}, reason: "${reason}".`;
}

/** Render the watched-field list for the contrastive override condition. */
function watchList(freeze) {
  const w = freeze?.watch_fields || [];
  return w.length ? `[${w.join(', ')}]` : '[no watched fields]';
}
...
export async function loadAllCanonEntries(projectRoot) {
  ...
  try {
    ...
    // (a config typo shouldn't turn into "no one can generate/curate/paint
    // anything in this project"). But that degrade-open MUST be audible —
    // silently disabling freeze protection project-wide with zero operator
    // signal is its own defect. Warn loudly to stderr; the operator will
    // also hit the same config error surfaced properly on `sdlab canon build`.
    warn(
      `freeze gate disabled for this project: canon-build/config.json is invalid ` +
        `(${err.message}). Every entry is being treated as not canon-bound — freeze/soft-advisory ` +
        `protection is OFF until the config is fixed. Run "sdlab canon build" to see the full error.`,
    );
    return [];
  }
  const entries = [];
...
export async function assertNotFrozen({
  ...
  // Contrastive refuse messaging (Horvitz CHI 1999): lead with the default
  // action + the system's reasoning (who/when/why) pulled from the freeze
  // stamp, then state the override path and its condition (don't touch the
  // watched fields). frozenAt is best-effort from the event log.
  if (status === 'frozen' || status === 'soft-advisory') {
    const frozenAt = await latestFreezeAt(projectRoot, id);
    const lead = contrastiveLead(id, status, fm.freeze, frozenAt);
    const watched = watchList(fm.freeze);

    if (status === 'frozen') {
      throw inputError(
        'CANON_ENTRY_FROZEN',
        `${lead} A hard freeze has no inline bypass, so "${action}" will not proceed.`,
        `Unfreeze with: sdlab canon unfreeze ${id} --reason "<why>" (appended to canon-build/freeze-events.jsonl as an audit record). This freeze watches ${watched}.`,
      );
    }
    ...
    if (!allowSoftAdvisoryBypass) {
      throw inputError(
        'CANON_ENTRY_FROZEN',
        `${lead} "${action}" has no --i-know bypass.`,
        `Run: sdlab canon unfreeze ${id} --reason "<why>" for a proper unfreeze, or pick a command that accepts --i-know. Watched fields: ${watched}.`,
      );
    }
    if (!bypassReason) {
      throw inputError(
        'CANON_ENTRY_FROZEN',
        `${lead} Override requires --i-know AND --reason.`,
        `Re-run with --i-know --reason "<why>" ONLY if your change does not touch the watched fields ${watched}. The bypass is logged to canon-build/freeze-events.jsonl.`,
      );
    }
