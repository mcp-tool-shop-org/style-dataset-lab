...
export function getProjectName(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--project' || a === '--game') {
      const flagName = a === '--game' ? 'game' : 'project';
      if (a === '--game') warnDeprecated('game', 'project');
      const next = argv[i + 1];
      // SDL-L1: mirror takeFlagValue's guard below — without it, a missing
      // value silently swallowed the NEXT flag as the project name (e.g.
      // `--project --dry-run` yielded the project name "--dry-run").
      if (next === undefined) {
      ...
      if (next.startsWith('--')) {
        throw inputError(
          'INPUT_MISSING_VALUE',
          `Flag --${flagName} is missing its value (got another flag: ${next}).`,
