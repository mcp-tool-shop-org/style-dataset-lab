 ...
 */
async function loadAdapter(adapterTarget) {
  const adapterPath = `./adapters/${adapterTarget}.js`;
  try {
    return await import(adapterPath);
  } catch {
    throw new Error(
      `Adapter "${adapterTarget}" not found. Available: generic-image-caption, diffusers-lora`
    );
  }
}
...
export async function buildTrainingPackage(projectRoot, manifestId, options = {}) {
  ...
  // Write package manifest (last — references everything)
