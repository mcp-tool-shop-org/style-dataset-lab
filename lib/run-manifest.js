 ...
 * the kind-appropriate subdirs first, then every known subdir, then the root.
 * Returns null when nothing matches (unresolvable → sha256 null with a note).
 */
export function resolveModelPath(name, kind, modelsDir = defaultModelsDir()) {
  ...
  const subdirs = [
    ...(KIND_SUBDIRS[kind] || []),
    ...Object.values(KIND_SUBDIRS).flat(),
    '.',
  ];
 ...
 * `doHash` (the --hash-models opt-in) and cached by (name, size, mtime) so
 * multi-GB checkpoints aren't re-hashed every wave.
 *
...
export function hashModelFile(name, { kind, modelsDir = defaultModelsDir(), doHash = false, cache } = {}) {
  ...
  if (!doHash) return { name, size_bytes, sha256: null, hash_note: 'hash skipped (pass --hash-models)' };
  const key = `${name}:${size_bytes}:${Math.round(mtimeMs)}`;
...
export function buildPinning({ graph, models = {}, loras = [], seedPolicy = null, hashModels = false, modelsDir = defaultModelsDir(), cache } = {}) {
  ...
  return {
    ...
    models: buildModelHashes(models, opts),
    loras: (loras || []).map((l) => ({
