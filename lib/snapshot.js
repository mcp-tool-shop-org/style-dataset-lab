...
export async function createSnapshot(projectRoot, profile, options = {}) {
  ...
  for (const file of files) {
    ...
    const { eligible, reasons, reason_codes: reasonCodes } = evaluateEligibility(
    ...
    if (eligible) {
      ...
      // F5: forward the stable reason_codes alongside the human-readable
      // reasons — purely additive on excluded.jsonl's on-disk shape (one
      // new key), so old readers/old files that only expect
      // {record_id, reasons} are unaffected. Lets categorizeExclusions
      // (lib/eligibility.js) categorize this snapshot's exclusions by
      // code instead of prose-matching `reasons`.
      excluded.push({ record_id: record.id, reasons, reason_codes: reasonCodes });
    }
  }
