...
export function buildRunSummary(manifest) {
  // B10: defensive default — manifest.errors two lines below already had
  // one; a legacy/truncated manifest with no `outputs` field crashed here
  // with a bare TypeError instead of degrading to an empty results view.
...
export function renderRunText(manifest) {
  // B10: same defensive default as buildRunSummary above — not explicitly
  // named by line number in the finding, but the identical defect in the
  // same file (this is the function `sdlab run show` calls in text mode).
