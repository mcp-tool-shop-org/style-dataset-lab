...
// SDL-H7: the only shape a batch id can ever legitimately have, mirroring
// the prefix/date/seq scheme generateBatchId() builds above. Validated
// BEFORE the id is joined into a path, so `--resume
// ../../../../some/dir/leftover` never reaches existsSync/readFile —
// executeBatchRuns checkpoints into batchDir/manifest.json after every
// slot, so an unvalidated id here meant repeatedly overwriting a file
// outside the sandbox. A mismatch reuses the SAME BATCH_NOT_FOUND code (not
// a distinct one) because no real batch can ever have an id outside this
// shape, and this avoids leaking validation internals.
const BATCH_ID_RE = /^batch_\d{4}-\d{2}-\d{2}_\d{3}$/;
 ...
 *
 * Throws BATCH_NOT_FOUND if the batch dir or manifest is missing (or the id
 * is not shaped like a real batch id).
