...
import { computeConfigFingerprint, SCHEMA_VERSION, checkManifestVersion, claimIdDir } from './snapshot.js';
...
export async function buildEvalPack(projectRoot, options = {}) {
  ...
  // Load all records (DB-003-adjacent: skip unparseable files rather than
  // aborting the entire eval-pack build — same graceful-degradation shape
  // snapshot.js uses).
  const records = [];
  for (const file of files) {
    try {
      ...
      // Skip unreadable records; eval pack is a best-effort selection and
      // one bad file must not kill the whole build.
    }
  }
  ...
  // Write eval pack
  // SDL-M10: atomically claim the eval-pack id/dir — closes the
  ...
  const { id: evalId, dir: evalDir } = await claimIdDir(
    join(projectRoot, 'eval-packs'),
    generateEvalId,
    {
      code: 'EVAL_PACK_ID_COLLISION',
      message: 'Could not claim a unique eval-pack ID — every candidate collided with an existing eval-pack directory.',
      hint: 'Retry; if this persists, another sdlab process may be creating eval packs in a tight loop.',
    },
  );

  // DB-014: compute `records_available` so the manifest reports sampled vs.
  // available. Operators can see when they're leaving records on the table.
  const laneAvailable = countLaneCoverageCandidates(records, config);
  const driftAvailable = countForbiddenDriftCandidates(records);
  const anchorAvailable = countAnchorGoldCandidates(records, config);
  const continuityAvailable = countSubjectContinuityCandidates(records);
  ...
  const manifest = {
    ...
    // D-017: capture config fingerprint so eval verdicts can detect canon drift.
    ...
    // DB-014: coverage — sampled vs. available per task
    records_sampled: {
      lane_coverage: laneCoverage.length,
      forbidden_drift: forbiddenDrift.length,
      anchor_gold: anchorGold.length,
      subject_continuity: subjectContinuity.length,
      total: laneCoverage.length + forbiddenDrift.length +
        anchorGold.length + subjectContinuity.length,
    },
    records_available: {
      lane_coverage: laneAvailable,
      forbidden_drift: driftAvailable,
      anchor_gold: anchorAvailable,
      subject_continuity: continuityAvailable,
      total: laneAvailable + driftAvailable + anchorAvailable + continuityAvailable,
    },
    caps: {
      max_per_lane: maxPerLane,
      max_per_faction: maxPerFaction,
      max_drift: maxDrift,
      max_continuity: maxContinuity,
    },
...
/** DB-014: count approved+canon-bound records (the lane-coverage candidate pool). */
function countLaneCoverageCandidates(records, _config) {
  let n = 0;
  for (const r of records) {
    if (r.judgment?.status === 'approved' && r.canon?.assertions?.length) n++;
  }
  return n;
}

/** DB-014: count records that qualify as forbidden-drift candidates. */
function countForbiddenDriftCandidates(records) {
  let n = 0;
  for (const r of records) {
    const status = r.judgment?.status;
    if (status !== 'rejected' && status !== 'borderline') continue;
    const failureModes = r.judgment?.failure_modes || [];
    const failedAssertions = (r.canon?.assertions || [])
      .filter(a => a.verdict === 'fail').length;
    if (failureModes.length > 0 || failedAssertions > 0 || r.judgment?.explanation) n++;
  }
  return n;
}

/** DB-014: count approved+canon-bound records (also anchor-gold pool). */
function countAnchorGoldCandidates(records, _config) {
  return countLaneCoverageCandidates(records, _config);
}

/** DB-014: count named subjects (subject-continuity pool). */
function countSubjectContinuityCandidates(records) {
  const subjects = new Set();
  for (const r of records) {
    const s = r.identity?.subject_name;
    if (s) subjects.add(s);
  }
...
export async function loadEvalPack(projectRoot, evalId) {
  ...
  checkManifestVersion(manifest, 'eval-pack');
  return manifest;
}
