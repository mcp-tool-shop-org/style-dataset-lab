...
/**
 * Derive the style trigger token from a profile.
 *
 * If `profile.trigger_override` is a non-empty string, use it verbatim.
 * Otherwise fall back to the hyphenated-id derivation:
 * `character-style-lora` → `character_style_lora`.
 *
 * `trigger_override` exists so profile IDs can stay descriptive (e.g.
 * `character-style-lora-flux`) while triggers stay compact, spec-compliant
 * (`^[a-z0-9_]+$`), and game-prefixed. Validation of the override value
 * (format + generic-suffix deny-list) lives in training-profiles.js so
 * captions are never the gate for a malformed trigger.
 *
 * @param {{profile_id?: string, trigger_override?: string}} profile
 * @returns {string|null}
 */
export function deriveStyleTrigger(profile) {
  if (!profile) return null;
  if (typeof profile.trigger_override === 'string' && profile.trigger_override.length > 0) {
    return profile.trigger_override;
  }
...
export function buildCaption(record, lane, group, profile) {
  ...
  switch (strategy) {
    ...
    case 'subject-natural-language':
      return buildSubjectNaturalLanguageCaption(record, lane, group, profile);
 ...
 */
/**
 * Subject natural-language caption.
 *
 * Same trigger-first prose shape as `flux-natural-language`, but the scene
 * text comes from `record.canon.subject` — an authored, style-free content
 * line — instead of `judgment.explanation`.
 *
 * Why this exists: `judgment.explanation` is a CURATION field. On projects
 * where curation notes record rulings ("Director ruling 2026-07-30: pool
 * look approved...") rather than scene descriptions, the two existing clean
 * strategies would bake review metadata into every caption. That is the same
 * class of defect as legacy prompt-bleed, just from a different field.
 *
 * `canon.subject` must carry CONTENT ONLY — no style vocabulary ("painterly
 * gouache", "sixteen-bit palette"), because style must attach to the trigger.
 * Style words in the subject teach the model to depend on the phrase instead
 * of the token.
 *
 * Shape: `<trigger> style, <subject>.`
 *
 * FAILS LOUD when no subject is present — it does NOT fall back to
 * `flux-natural-language`. That fallback used to read `judgment.explanation`
 * for the scene segment, which silently re-introduces exactly the prompt-bleed
 * antipattern this strategy exists to prevent, just sourced from a curation
 * field instead of `provenance.prompt`. A record selected for this strategy
 * with no `canon.subject` is a data-authoring gap, not a degrade-gracefully
 * case: it must be fixed upstream (populate `canon.subject`) or the record
 * excluded, never silently captioned from review metadata.
 */
function buildSubjectNaturalLanguageCaption(record, lane, group, profile) {
  const subject = record?.canon?.subject;
  if (typeof subject !== 'string' || subject.trim().length === 0) {
    const id = record?.id || '(unknown id)';
    throw inputError(
      'CAPTION_SUBJECT_MISSING',
      `Record "${id}" uses caption_strategy "subject-natural-language" but has no ` +
        `canon.subject (content-only description). Refusing to caption from another field.`,
      `Populate canon.subject on record "${id}" with a style-free content line (no ` +
        `"painterly"/"gouache"/palette vocabulary — that belongs to the trigger, not the ` +
        `subject). Do not let this silently fall back to judgment.explanation, which is a ` +
        `curation field and may contain review rulings, not scene content.`,
    );
  }
  ...
  segments.push(subject.trim().replace(/\.$/, ''));
