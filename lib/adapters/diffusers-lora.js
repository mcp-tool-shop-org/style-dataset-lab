...
export async function buildPackage({ packageDir, rows, profile, manifest, config, projectRoot, copy }) {
  ...
  for (const [partition, partitionRows] of Object.entries(partitioned)) {
    ...
    for (const row of partitionRows) {
      ...
      assertSafeId(row.entity_id, `row.entity_id in diffusers-lora adapter (partition "${partition}")`);
  ...
  if (imagesFailed.length > 0) {
    ...
    warn(
      `diffusers-lora adapter: ${imagesFailed.length} of ${imageCount + imagesFailed.length} rows failed to place` +
