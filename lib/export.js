...
export async function buildExport(projectRoot, snapshotId, splitId, options = {}) {
  ...
  // later deleted from records/ must never silently drop the export's
  // record count below snapshot.counts.included. loadRecord() returns
  // null on ENOENT (lib/records.js) — previously that null was a silent
  // `continue` with no counter, no warning, no id recorded, so an export
  // built from a snapshot with stale/deleted records would just quietly
  // contain fewer rows than snapshot.counts.included promised. Accumulated
  // across the whole loop (not fail-fast on the first) so the operator
  // sees every missing id in one error instead of fixing them one at a
  // time; hard-failed below rather than warned — see the comment at the
  // throw site for why export specifically earns that escalation.
  ...
  for (const entry of included) {
    ...
    if (!record) {
      missingRecords.push({ record_id: entry.record_id, stage: 'export' });
      continue;
    }
  ...
  // F3: hard-fail on any record the snapshot promised but records/ no
  // longer has. Nothing has been written to exportDir yet (metadata.jsonl,
  // images/, manifest.json, etc. are all written below this point) — an
  // aborted export leaves only the empty claimed directory behind, no
  // partial/misleading artifacts. Export (and training-packages.js one
  // layer further downstream) is where Law 4 ("exports are reproducible —
  // rebuildable from snapshot + split + profile") makes completeness an
  // actual contract rather than a courtesy, which is why this escalates to
  // a thrown error instead of the warn()-and-continue ledger pattern used
  // in split.js and implementation-packs.js, where the artifact produced
  // is not itself the reproducibility-guaranteeing one.
  if (missingRecords.length > 0) {
    const sample = missingRecords.slice(0, 10).map(m => m.record_id);
    throw inputError(
      'EXPORT_MISSING_RECORDS',
      `${missingRecords.length} of ${included.length} record(s) referenced by snapshot "${snapshotId}" could not be loaded from ${recordsDir} — this export would silently contain fewer records than the snapshot's included count.` +
      (missingRecords.length > 10
        ? ` First 10: ${sample.join(', ')}, ...`
        : ` Missing: ${sample.join(', ')}.`),
      'Record file(s) were likely deleted after the snapshot was created (a snapshot freezes the SELECTION, not the underlying record files). Restore the missing file(s), or create a fresh snapshot that reflects current on-disk records before exporting.',
    );
  }
