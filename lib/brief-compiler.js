...
function computeBriefFingerprint(projectRoot, workflow, subjectRecords) {
  ...
  // Workflow profile (serialized deterministically)
  hash.update('--- workflow ---\n');
  hash.update(JSON.stringify(workflow, Object.keys(workflow).sort()));
  hash.update('\n');
 ...
 */
function buildRuntimePlan(workflow, overrides = {}) {
