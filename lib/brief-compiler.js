...
export async function compileBrief({
  ...
  // H1: subject_mode 'required' is enforced above on the *string* being
  // non-empty (BRIEF_SUBJECT_REQUIRED), but resolveSubject() swallows every
  // per-file parse failure (catch {}) and returns [] when nothing matches —
  // a typo'd subject id passed that string check and silently compiled a
  // generic brief wearing a subject label it never anchored to (no
  // subject_identity block, no subject_constraints, both gated on
  // length > 0 below). resolveReferences() fails loudly (BRIEF_REF_NOT_FOUND)
  // on the identical "nothing matched" case — this closes the asymmetry.
  if (subjectMode === 'required' && subjectRecords.length === 0) {
    throw inputError(
      'BRIEF_SUBJECT_NOT_FOUND',
      `Workflow "${workflowId}" requires a subject (subject_mode: required), but no records matched ` +
        `subject "${subjectId}" in ${recordsDir}.`,
      `Check "${subjectId}" for typos, or confirm records exist with identity.subject_id (or an id ` +
        `prefix) matching it. resolveSubject() matches by identity.subject_id equality or by id-prefix — ` +
        `see lib/brief-compiler.js's resolveSubject().`
    );
  }
