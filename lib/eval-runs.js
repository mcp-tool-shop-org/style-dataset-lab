...
export async function scoreEvalRun(projectRoot, evalRunId, outputsPath) {
  ...
  // F2: createEvalRun only ever stores the eval_pack_id string — it never
  // re-verifies at scoring time that the pack directory still exists. If
  // the pack was deleted (or moved) between run creation and scoring, this
  // used to fall straight through to the all-four-skip / vacuous-pass path
  // below. Fail loudly here with the exact missing path instead.
  if (!existsSync(evalPackDir)) {
    throw inputError(
      'EVAL_PACK_DIR_MISSING',
      `Eval pack directory not found: ${evalPackDir} (eval run "${evalRunId}" references eval_pack_id "${runManifest.eval_pack_id}").`,
      'The eval pack this run was created against may have been deleted or moved. Rebuild the eval pack, or create a fresh eval run against one that still exists.',
    );
  }
  const taskFiles = {
  ...
  const scores = {};
  // F2: count task files that actually resolved. Object.values({}).every(...)
  // on an EMPTY scores object is vacuously true in JS — with zero task files
  // found, `scores` stays `{}` and the verdict below silently computed
  // overall_verdict:'pass', overall_score:0.000 for a model that was never
  // evaluated against anything. resolvedTaskFiles turns that into a named,
  // structured failure instead of a green rubber stamp.
  let resolvedTaskFiles = 0;
  ...
  // F2: belt-and-suspenders — even with the directory-existence guard above,
  // a pack directory that exists but has had every individual task file
  // deleted out from under it must hit the same guard, not the vacuous
  // Object.values({}).every(...) === true path.
  if (resolvedTaskFiles === 0) {
    throw inputError(
      'EVAL_PACK_NO_TASK_FILES',
      `Eval pack "${runManifest.eval_pack_id}" at ${evalPackDir} has no resolvable task files — nothing was scored for eval run "${evalRunId}".`,
      `Expected at least one of: ${Object.values(taskFiles).join(', ')}. Verify the eval pack was built correctly and its directory was not partially deleted.`,
    );
  }
