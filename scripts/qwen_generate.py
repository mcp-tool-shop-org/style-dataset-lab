...
def qwen_graph(pos: str, neg: str, seed: int, w: int, h: int, steps: int, cfg: float, prefix: str) -> dict:
    return {
...
def generate(pos: str, neg: str, seed: int, dst: Path, **kw) -> Path:
    g = qwen_graph(pos, neg, seed, kw["w"], kw["h"], kw["steps"], kw["cfg"], dst.stem)
...
def main() -> None:
    ...
    receipt = {"wave": wave.get("wave"), "base": "qwen-image", "unet": QWEN_UNET, "clip": QWEN_CLIP,
               ...
               "steps": steps, "cfg": cfg, "size": [w, h], "style_prefix": prefix, "negative": neg, "items": []}
    seed = base_seed
    for subj in wave["subjects"]:
        ...
        for v in range(nvar):
            ...
            generate(pos, neg, seed, dst, w=w, h=h, steps=steps, cfg=cfg)
