...
const REJECT_EXPLANATIONS = {
  ...
  "REJECT6_fashion_model_f": { explanation: "Haute couture space-themed evening gown with crystals and high heels. Fashion runway not shipboard life. Violates every art pillar.", failures: ["costume_not_clothes", "too_clean"], scores: { silhouette_clarity: 0.8, palette_adherence: 0.1, material_fidelity: 0.2, faction_read: 0.0, wear_level: 0.0, style_consistency: 0.3, clothing_logic: 0.0, composition: 0.8 } },
  "REJECT6_steampunk_m": { explanation: "Victorian steampunk with top hat, brass goggles, gears and cogs, steam-powered prosthetic. Retro-futurism cosplay not lived-in sci-fi. Wrong aesthetic entirely.", failures: ["costume_not_clothes", "generic_scifi"], scores: { silhouette_clarity: 0.8, palette_adherence: 0.3, material_fidelity: 0.4, faction_read: 0.0, wear_level: 0.4, style_consistency: 0.3, clothing_logic: 0.1, composition: 0.8 } },
  "REJECT6_identical_twins": { explanation: "Two identical figures in matching pristine uniforms. Violates single character rule and too-clean rule. Looks like mannequins or clones.", failures: ["bad_composition", "too_clean"], scores: { silhouette_clarity: 0.5, palette_adherence: 0.6, material_fidelity: 0.5, faction_read: 0.5, wear_level: 0.1, style_consistency: 0.5, clothing_logic: 0.4, composition: 0.2 } },
  "REJECT7_veshan_human_clothes": { explanation: "Veshan in Compact-style pressed uniform designed for human body. Fights against scale ridges and digitigrade legs. Wrong material vocabulary for species.", failures: ["wrong_material", "faction_unreadable"], scores: { silhouette_clarity: 0.7, palette_adherence: 0.5, material_fidelity: 0.2, faction_read: 0.2, wear_level: 0.6, style_consistency: 0.6, clothing_logic: 0.2, composition: 0.8 } },
  "REJECT7_cute_dragon": { explanation: "Adorable small dragon with sparkly eyes, tiny wings, pink pastel scales, cute crown and cape with stars. Mascot toy not warrior civilization member.", failures: ["faction_unreadable", "costume_not_clothes"], scores: { silhouette_clarity: 0.7, palette_adherence: 0.1, material_fidelity: 0.2, faction_read: 0.0, wear_level: 0.1, style_consistency: 0.2, clothing_logic: 0.1, composition: 0.8 } },
  "REJECT8_superhero_f": { explanation: "Glossy red and gold spandex bodysuit with cape, mask, power pose, stylized emblem. Superhero costume not clothing. Wrong genre entirely.", failures: ["costume_not_clothes", "generic_scifi"], scores: { silhouette_clarity: 0.8, palette_adherence: 0.1, material_fidelity: 0.2, faction_read: 0.0, wear_level: 0.0, style_consistency: 0.2, clothing_logic: 0.0, composition: 0.7 } },
  "REJECT8_zombified_m": { explanation: "Rotting grey skin, milky eyes, bloodied torn uniform. Horror zombie genre not lived-in sci-fi. Wrong genre.", failures: ["generic_scifi"], scores: { silhouette_clarity: 0.7, palette_adherence: 0.3, material_fidelity: 0.4, faction_read: 0.3, wear_level: 0.5, style_consistency: 0.3, clothing_logic: 0.3, composition: 0.7 } },
  "REJECT9_invisible_m": { explanation: "Cloaking suit making the character transparent. Cannot judge style of something invisible. Science fiction special effect not costume.", failures: ["generic_scifi"], scores: { silhouette_clarity: 0.1, palette_adherence: 0.2, material_fidelity: 0.1, faction_read: 0.0, wear_level: 0.0, style_consistency: 0.3, clothing_logic: 0.1, composition: 0.5 } },
...
async function main() {
  ...
  for (const file of files.filter(f => f.endsWith(".json")).sort()) {
    ...
    if (REJECT_EXPLANATIONS[baseId]) {
      ...
      scores = rej.scores;
    } else if (baseId.startsWith("BORDERLINE_")) {
      status = "borderline";
      explanation = `Almost on-style but fails in a subtle way that teaches nuance. See subject ID for specific issue.`;
      scores = { silhouette_clarity: 0.7, palette_adherence: 0.65, material_fidelity: 0.65, faction_read: 0.6, wear_level: 0.6, style_consistency: 0.65, clothing_logic: 0.6, composition: 0.75 };
    } else {
