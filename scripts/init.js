...
// `projects/` is the user's own work, so it is workspace-relative. In a repo
// checkout these coincide, which is why resolving both against REPO_ROOT
// passed every in-repo test while breaking `init` for every installed user.
const TEMPLATES_DIR = join(getPackageRoot(), 'templates');
...
export async function run(argv = process.argv.slice(2)) {
  ...
  // SDL-M6: the generate → curate → reingest loop fills inbox/generated/ and
  // the outputs/ subtrees with PNGs (see lib/reingest-selected.js), but this
  // repo's OWN .gitignore (which happens to cover those paths in dev) is not
  // in the npm `files` allow-list and ships to nobody. Without a per-project
  // .gitignore, every downstream `sdlab init` user stages regenerable blobs
  // by construction — measured at 55 MB of untracked PNGs in this repo before
  // this fix. Never clobber a .gitignore the user already created by hand.
  const gitignorePath = join(projectDir, '.gitignore');
  if (!existsSync(gitignorePath)) {
    const gitignoreLines = [
      '# Generated by `sdlab init` — regenerable blobs for this project.',
      '# Records/config JSON are tracked; only pixels and derived artifacts are not.',
      '',
      'inbox/**/*.png',
      'inbox/**/*.jpg',
      'inbox/**/*.jpeg',
    ...
    console.log(`  \x1b[32m✓\x1b[0m .gitignore (regenerable outputs/inbox/exports/snapshots/splits)`);
  }
