...
test('the version gate accepts 1.2.0 and still refuses a higher minor loudly', () => {
  assert.equal(ASSET_SCHEMA_VERSION, '1.2.0');
  const ok = shapeViolations((m) => { m.schema_version = '1.2.0'; });
  assert.deepEqual(ok, []);
  const tooNew = shapeViolations((m) => { m.schema_version = '1.3.0'; });
