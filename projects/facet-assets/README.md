...
sdlab project doctor --project facet-assets

# Generate candidates from the example prompt pack
sdlab generate inputs/prompts/example-wave.json --project facet-assets

# Curate a candidate (approved | rejected | borderline)
sdlab curate <asset_id> approved "reads clean" --project facet-assets

# Bind approved records to constitution rules
sdlab canon-bind --project facet-assets

# Create a frozen dataset snapshot
sdlab snapshot create --project facet-assets
```
