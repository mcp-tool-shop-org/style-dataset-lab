...
A `style-dataset-lab` project — domain: **game-art**.

## Quick commands

```bash
# Validate that the scaffold is healthy
sdlab project doctor --project salt-road

# Generate candidates from the example prompt pack
sdlab generate inputs/prompts/example-wave.json --project salt-road

# Curate a candidate (approved | rejected | borderline)
sdlab curate <asset_id> approved "reads clean" --project salt-road

# Bind approved records to constitution rules
sdlab canon-bind --project salt-road

# Create a frozen dataset snapshot
sdlab snapshot create --project salt-road
```
