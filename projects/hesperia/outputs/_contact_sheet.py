...
out = Path(r"E:/AI/style-dataset-lab/projects/hesperia/outputs/world-plates-w1-v3")
subjects = [("vertical_city", 4), ("the_towers", 4), ("under_city_hub", 4), ("rust_communion", 4),
            ...
            ("landed_ark", 4), ("dead_sky_hesperos", 3), ("sams_road", 3)]
...
for sid, nvar in subjects:
    ...
    y += LBL
    for v in range(nvar):
        p = out / f"{sid}_v{v}.png"
        if p.exists():
            im = Image.open(p).convert("RGB").resize((TW, TH))
            sheet.paste(im, (v * TW, y))
            draw.text((v * TW + 5, y + 3), f"v{v}", fill=(255, 228, 110), font=font)
    y += TH

dst = out / "_contact_sheet.png"
sheet.save(dst)
print("contact sheet:", dst, sheet.size)
